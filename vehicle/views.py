...
def is_admin(user):
    return not user.groups.filter(name__in=['CUSTOMER', 'MECHANIC']).exists() and user.is_staff
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if is_mechanic(user):
                ...
                form.add_error(None, 'You are not authorized as a mechanic.')
    else:
        form = AuthenticationForm()
...
def logout_view(request):
    logout(request)
    return redirect('afterlogin')   # or 'afterlogin'